...
const Collection = () => {
  ...
  return (
    <div className="pt-32 container-custom min-h-screen flex flex-col sm:flex-row gap-1 sm:gap-10 border-t border-gray-100">
      
      {/* Filter Sidebar */}
      <div className="min-w-60 py-10">
        <div className="flex items-center justify-between cursor-pointer sm:cursor-default" onClick={() => setShowFilter(!showFilter)}>
            <p className="text-xl font-serif mb-4 uppercase tracking-wider">Filters</p>
            <img className={`h-3 sm:hidden transition-transform ${showFilter ? 'rotate-90' : ''}`} src={assets.dropdown_icon} alt="" />
        </div>
        
        <div className={`flex flex-col gap-8 transition-all duration-300 overflow-hidden ${showFilter ? 'max-h-[500px] opacity-100' : 'max-h-0 opacity-0 sm:max-h-full sm:opacity-100'}`}>
            {/* Category Filter */}
            <div className="border border-gray-200 p-5 rounded-sm">
                <p className="mb-3 text-sm font-bold uppercase tracking-widest">Category</p>
      ...
      <div className="flex-1 py-10">
        <div className="flex justify-between items-center mb-8">
            <h1 className="text-2xl font-serif uppercase tracking-wide">All Collections</h1>
            
            {/* Sort */}
            <select onChange={(e) => setSortType(e.target.value)} className="border border-gray-200 text-sm px-2 py-2 rounded-sm focus:outline-none focus:border-black cursor-pointer">
